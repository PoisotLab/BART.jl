...
function probgrow(d::Int64, bm::BartModel)
    return bm.hypers.α * (1.0 + d)^(-bm.hypers.β)
end
...
function log_tree_prior(tree::Tree, bm::BartModel)
    ...
    return log_tree_prior(tree.root.left, tree, bm, lp) +
...
function leafprob(
    ...
    return leafprob(x, branch.right, tree, goesright, S)
end
...
function birthprob(tree::Tree)
    return isa(tree.root, Leaf) ? 1.0 : 0.5
end

function birthprob(S::Matrix{Float64})
    return size(S, 2) == 1 ? 1.0 : 0.5
end
...
function log_death_tree(node::Node, tree::Tree, bm::BartModel)
    return -1.0 * log_birth_tree(node, tree, bm)
end
...
function drawλ!(bt::BartTree, rt::Vector{Float64}, bs::BartState, bm::BartModel)
    ...
    ss_prime = suffstats(rt, S_prime, bs, bm)
    logr =
        mll(rt, ss_prime, bs, bm) + lp_λprime + log_λprime -
...
function drawσ!(bs::RegBartState, bm::BartModel)
    ...
    return bs.σ = sqrt(rand(InverseGamma(a, b)))
end
...
function varcount!(branch::Branch, counts::Vector)
    ...
    return varcount!(branch.right, counts)
end
...
function loglikω(ω, log_s, a, b, p)
    aω = p * (ω / (1 - ω))
    return loggamma(aω) - p * loggamma(aω / p) + aω * mean(log_s) + logpdf(Beta(a, b), ω)
end
...
function ptd(bc::BART.ProbitBartChain)
    ...
    return pmap(_ptd, [[trees, bc.bm] for trees in treedraws])
end
